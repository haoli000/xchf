...
main() {
	...
	release_uri="https://github.com/haoli000/xchf/releases/download/$version/tttne_${os}_${arch}.tar.gz"
	...
	ts=$(date +%s)
	tmp_dir="$install_path/xchf_tmp$ts"
	exe="$bin_dir/xchf"
	...
	if command -v xchf >/dev/null; then
		echo "Run 'xchf help' to get started"
	else
		case $SHELL in
